...
queue "chores" {
  type = "external"
  list = "wok list -t chore -s todo --unassigned -p oj -o json"
...
pipeline "chore" {
  ...
  step "reopen" {
    run = "wok reopen ${var.task.id} --reason 'Chore pipeline failed'"
  }

  step "cancel" {
    run = "wok close ${var.task.id} --reason 'Chore pipeline cancelled'"
  }
}
