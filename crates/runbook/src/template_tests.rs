...
// =============================================================================
// interpolate_shell_trusted tests
// =============================================================================

#[test]
fn interpolate_shell_trusted_skips_escaping_for_trusted_prefix() {
    let vars: HashMap<String, String> = [(
    ...
    .collect();
    assert_eq!(
        interpolate_shell_trusted(r#"git -C "${local.repo}" worktree add"#, &vars, &["local."]),
        r#"git -C "$(git rev-parse --show-toplevel)" worktree add"#
    );
}

#[test]
fn interpolate_shell_trusted_still_escapes_untrusted() {
    let vars: HashMap<String, String> =
        [("var.title".to_string(), "fix: handle $PATH".to_string())]
            .into_iter()
            .collect();
    assert_eq!(
        interpolate_shell_trusted(r#"git commit -m "${var.title}""#, &vars, &["local."]),
        r#"git commit -m "fix: handle \$PATH""#
    );
}

#[test]
fn interpolate_shell_trusted_mixed_trusted_and_untrusted() {
    let vars: HashMap<String, String> = [
        (
            "local.repo".to_string(),
            "$(git rev-parse --show-toplevel)".to_string(),
    ...
    .collect();
    assert_eq!(
        interpolate_shell_trusted(
            r#"git -C "${local.repo}" push origin "${local.branch}" && echo "${var.title}""#,
            &vars,
            &["local."]
        ),
        r#"git -C "$(git rev-parse --show-toplevel)" push origin "fix/bug-123" && echo "fix: handle \"\$HOME\" path""#
    );
}

#[test]
fn interpolate_shell_trusted_empty_prefixes_escapes_all() {
    let vars: HashMap<String, String> =
        [("local.repo".to_string(), "$(git rev-parse)".to_string())]
            .into_iter()
            .collect();
    // With no trusted prefixes, behaves like interpolate_shell
    assert_eq!(
        interpolate_shell_trusted(r#"echo "${local.repo}""#, &vars, &[]),
        r#"echo "\$(git rev-parse)""#
    );
}
